        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {categories.map((category) => (
            <label key={category.id} className="group relative cursor-pointer">
              ...
              />
              <div className={`h-full border-2 rounded-xl p-6 transition-all duration-200 group-hover:border-primary/50 group-hover:shadow-xl group-hover:shadow-primary/5 ${
                selectedCategories.includes(category.id)
                  ? 'border-primary bg-primary/5 dark:bg-primary/10 border-slate-200 dark:border-slate-800'
                  : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'
              }`}>
                <div className="mb-4 w-12 h-12 rounded-lg bg-primary/10 dark:bg-primary/20 flex items-center justify-center text-primary transition-transform group-hover:scale-110">
